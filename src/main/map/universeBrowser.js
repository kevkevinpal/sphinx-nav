...
function UniverseBrowser(props) {
  ...
  function lookAt(x1, y1, z1) {
    ...
    cameraControls.setTarget(x1, y1, z1, true);
  }
  ...
  function rotateWorld() {
    ...
    rotateCycle = gsap.to(cameraControls, {
  ...
  const nodeObject = (node) => {
    ...
    // if (!img.includes('_s')) console.log('FAIL!', img)

    if (nodeMaterials[img]) {
      material = nodeMaterials[img].clone();
    } else {
      const map = loader.load('https://stakwork-uploads.s3.amazonaws.com/knowledge-graph-joe/content-images/4e607036-8fde-48ef-ab76-9c2ac4bab031_s.jpg');
      ...
      <Shield style={{ opacity: loading ? 1 : 0 }}>
        <div style={{ display: "flex" }}>
          ...
          <div style={{ marginLeft: 10 }}>
            Mapping {props.graphData?.nodes?.length} results...
          </div>
        </div>
      </Shield>
...
function areEqual(prevProps, nextProps) {
  ...
  const { width, height, graphData, currentSearchTerm, focusedNode } =
    prevProps;
